...
void ClayUILayer::Tick(float DeltaTime) {
    ...
    if (bClayInitialized) {
        ...
        if (tickDefinitionFunction) {
            ...
            const Clay_Color COLOR_DARK_ORANGE = Clay_Color{200, 100, 0, 255};
            const Clay_Color COLOR_BRIGHT_ORANGE = Clay_Color{255, 165, 0, 255};
            const Clay_Color COLOR_BLUEGREEN = Clay_Color{0, 128, 128, 255};
            CLAY({.id = CLAY_ID("OuterContainer"),
                ...
                CLAY({
                    .id = CLAY_ID("SideBar"),
                    .layout = {.sizing = {.width = CLAY_SIZING_FIXED(300), .height = CLAY_SIZING_GROW(0)},
                               .padding = CLAY_PADDING_ALL(16),
                               .childGap = 16,
                    ...
                    .border = {.color = COLOR_BLUEGREEN, .width = {5, 5, 10, 5}},
                }) {
                    CLAY({.id = CLAY_ID("ProfilePictureOuter"),
                          .layout = {.sizing = {.width = CLAY_SIZING_GROW(0)},
                                     .padding = Clay_Padding(8, 0, 8, 8),
                                     .childGap = 16,
                                     .childAlignment = {.y = CLAY_ALIGN_Y_CENTER}},
                          .backgroundColor = COLOR_RED}) {
                        CLAY({.id = CLAY_ID("ProfilePicture"),
                              .layout = {.sizing = {.width = CLAY_SIZING_FIXED(60), .height = CLAY_SIZING_FIXED(60)}},
                              .cornerRadius = {30, 15, 30, 15},
                              .image = {.imageData = CLAY_IMAGE_STRING("assets/images/profile.jpg")}}) {}
                        CLAY_TEXT(CLAY_STRING("Clay - UI Library - LLet's see how it handles "
                                              "much longer "
                                              "thingsLet's see how it handles much longer "
                                              "thingsLet's see "
                                              "how it handles much longer thingsLet's see how it "
                                              "handles "
                                              "much longer thingsLet's see how it handles much "
                                              "longer "
                                              "thingset's see how it handles much longer things"),
                                  CLAY_TEXT_CONFIG({
                    ...
                    for (int i = 0; i < 10; i++) {
                        auto bHovering = Clay_PointerOver(CLAY_IDI("BOX", i));
                        auto bPressed = bHovering && (mouseState == OZZ::EKeyState::KeyPressed);
                        Clay_Color bgColor = COLOR_ORANGE;
                        if (bPressed) {
                            bgColor = COLOR_DARK_ORANGE;
                        } else if (bHovering) {
                            bgColor = COLOR_BRIGHT_ORANGE;
                        }

                        CLAY({.id = CLAY_IDI("BOX", i),
                              .layout = {.sizing = {.width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(50)}},
                              .backgroundColor = bgColor}) {
                            CLAY_TEXT(CLAY_STRING("I can't believe this is working"),
                                      CLAY_TEXT_CONFIG({.textColor = randomColors[i],
                                                        .fontId = 1,
                                                        .fontSize = randomFontSizes[i],
                                                        .textAlignment = CLAY_TEXT_ALIGN_CENTER}));
                            // children go here...
                        }
                    }
