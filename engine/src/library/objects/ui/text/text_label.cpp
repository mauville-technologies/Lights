...
namespace OZZ::game::objects {
	TextLabel::TextLabel(GameWorld *inGameWorld, std::shared_ptr<OzzWorld2D> inPhysicsWorld, const ParamsType& inParams)
		: UIComponent(inGameWorld, std::move(inPhysicsWorld)), params(inParams), fontLoader(std::make_unique<OZZ::FontLoader>()) {
...
;	}
	...
	void TextLabel::updateText() {
		...
		if ((params.Text == builtText && bBuilt)) return;
		fontRenderObject = {};
		// create the texture
		fontTexture.reset();
	...
	void TextLabel::updateRectBounds() {
		...
		const bool bIsCenter = params.AnchorPoint == AnchorPoint::CenterTop ||
			params.AnchorPoint == AnchorPoint::CenterBottom || params.AnchorPoint == AnchorPoint::CenterMiddle;
		const bool bIsLeft = params.AnchorPoint == AnchorPoint::LeftTop ||
			params.AnchorPoint == AnchorPoint::LeftBottom || params.AnchorPoint == AnchorPoint::LeftMiddle;
		const bool bIsRight = params.AnchorPoint == AnchorPoint::RightTop ||
			params.AnchorPoint == AnchorPoint::RightBottom || params.AnchorPoint == AnchorPoint::RightMiddle;
