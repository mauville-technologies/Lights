...
namespace OZZ::game::objects {
    Button::Button(const uint64_t inId,
    ...
    bool Button::IsMouseOver(const glm::vec2& worldPos) {
        ...
        if (worldPos.x >= buttonPos.x - buttonSize.x / 2 && worldPos.x <= buttonPos.x + buttonSize.x / 2 &&
            worldPos.y >= buttonPos.y - buttonSize.y / 2 && worldPos.y <= buttonPos.y + buttonSize.y / 2) {
            return true;
        }
        return false;
    }
