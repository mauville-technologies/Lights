...
#include "lights/library/objects/ui/input/text_input.h"
...
namespace OZZ::game::objects {
    TextInput::TextInput(uint64_t inId,
        ...
        currentString = "";
        // create a text label
        label = gameWorld->CreateGameObject<TextLabel>(TextLabel::ParamsType{.FontPath = params.FontPath,
        ...
        vertices.push_back(Vertex{.position = {-params.Size.x / 2, -params.Size.y / 2, 0.f}});
        vertices.push_back(Vertex{.position = {params.Size.x / 2, -params.Size.y / 2, 0.f}});
        vertices.push_back(Vertex{.position = {-params.Size.x / 2, params.Size.y / 2, 0.f}});
        vertices.push_back(Vertex{.position = {params.Size.x / 2, params.Size.y / 2, 0.f}});
    ...
    void TextInput::SetupInput(InputSubsystem* inInputSubsystem) {
        ...
        inputSubsystem->RegisterInputMapping(
            {.Action = inputSID,
             .Chords = {InputChord{
                 .Keys = {{EDeviceID::Keyboard, EKey::BackSpace}}, .bIsSequence = false, .bCanRepeat = true}},
             .Callbacks = {.OnPressed = [this]() {
                 if (!IsFocused())
                     return;
                 removeCharacter();
             }}});
    ...
    bool TextInput::IsMouseOver(const glm::vec2& worldPos) {
        ...
        if (worldPos.x >= inputPos.x - inputSize.x / 2 && worldPos.x <= inputPos.x + inputSize.x / 2 &&
            worldPos.y >= inputPos.y - inputSize.y / 2 && worldPos.y <= inputPos.y + inputSize.y / 2) {
            ;
