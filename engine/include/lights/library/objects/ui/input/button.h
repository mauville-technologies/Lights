...
namespace OZZ::game::objects {
    ...
    public:
        ...
        struct ButtonParams {
            ...
            glm::vec3 CursorColor{1.f, 1.f, 1.f};

            glm::vec3 TextColor{1.f, 1.f, 0.f};

            glm::vec4 FocusedColor{1.f, 0.f, 0.5f, 1.f};

            glm::vec4 FocusedThickness{5.f};

            ClickCallback OnClick{nullptr};
            ButtonParams()
                : FontSize(32), FontPath(""), Text(""), Size(200.f, 50.f), BackgroundColor(0.2f, 0.2f, 0.2f, 1.f),
                  CursorColor(1.f, 1.f, 1.f), TextColor(1.f, 1.f, 0.f), FocusedColor(1.f, 0.f, 0.5f, 1.f),
                  FocusedThickness(5.f), OnClick(nullptr) {};
        };

        using ParamsType = ButtonParams;

        explicit Button(GameWorld* inGameWorld,
                        std::shared_ptr<OzzWorld2D> inPhysicsWorld,
                        const ParamsType& inParams = {});
        Button(uint64_t inId,
               GameWorld* inGameWorld,
               std::shared_ptr<OzzWorld2D> inPhysicsWorld,
               const ButtonParams& inParams);
        ~Button() override;
